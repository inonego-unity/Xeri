...
# 설명
UnityParticleSystemCueAsset의 Variant 선택 상태와 Variant별 ParticleSystem Pool을 소유하는 runtime Cue를 제공한다.
...
namespace inonego.Xeri.Playback
{
    // ============================================================
    /// <summary>
    /// Unity ParticleSystem 기반 runtime Visual Cue.
    /// </summary>
    // ============================================================
    ...
    #region 필드
        ...
        /// </summary>
        // ----------------------------------------------------------------------
        public UnityParticleSystemCueAsset Asset => asset;

        private readonly UnityParticleSystemCueAsset asset = null;
        private readonly UnityParticleSystemCueVariant standaloneVariant = null;
        private GOCompPool<ParticleSystem>[] pools = null;
    ...
    #region 생성자
        ...
        /// </summary>
        // ------------------------------------------------------------
        public UnityParticleSystemCue(UnityParticleSystemCueVariant variant)
            : base(excludePrevious: false)
        {
            standaloneVariant = variant ?? throw new ArgumentNullException(nameof(variant));
            pools = new GOCompPool<ParticleSystem>[1];
        }
        ...
        /// </summary>
        // ------------------------------------------------------------
        internal UnityParticleSystemCue(UnityParticleSystemCueAsset asset)
            : base(asset != null ? asset.ExcludePrevious : false)
        {
            this.asset = asset ?? throw new ArgumentNullException(nameof(asset));
            pools = new GOCompPool<ParticleSystem>[asset.VariantCount];
        }
    ...
    #region Variant 획득
        ...
        // ----------------------------------------------------------------------
        /// <summary>
        /// <br/> 이번 재생 Variant를 선택하고 대응 Pool에서 ParticleSystem Lease를 획득한다.
        /// <br/> 반환 Lease는 Cue의 활성 Lease 수명과 원본 Pool 반환을 함께 끝낸다.
        /// </summary>
        // ----------------------------------------------------------------------
        internal Lease<ParticleSystem> AcquireLease
        (
            Transform parent,
            Transform releasedRoot,
            out UnityParticleSystemCueVariant variant
        )
        {
            ThrowIfDisposed();
            ...
            return new Lease<ParticleSystem>
            (
                poolLease.Value,
        ...
        // ------------------------------------------------------------
        /// <summary>
        /// 이번 재생에 사용할 ParticleSystem Variant와 그 인덱스를 선택한다.
        /// </summary>
        // ------------------------------------------------------------
        private int SelectVariant(out UnityParticleSystemCueVariant variant)
        {
            ...
            if (asset.VariantCount <= 0)
            {
                ...
                (
                    $"Unity Particle System Cue Asset '{asset.name}'에 하나 이상의 Variant가 필요합니다."
                );
            }
            ...
            (
                $"Unity Particle System Cue Asset '{asset.name}'의 Variant {index}가 비어 있습니다."
            );
            return index;
        }
    ...
    #region 풀 연결과 생성
        ...
        private void EnsurePoolHost(Transform parent, Transform releasedRoot)
        {
            ...
            if (poolParent != null)
            {
                if (poolParent != parent || poolRoot != resolvedRoot)
                {
                    throw new InvalidOperationException
                    (
                        "하나의 UnityParticleSystemCue runtime 인스턴스를 서로 다른 살아있는 Player Host에 공유할 수 없습니다."
                    );
                }
        ...
        // ----------------------------------------------------------------------
        /// <summary>
        /// 지정 Variant 인덱스에 대응하는 ParticleSystem Pool을 조회하거나 최초 생성한다.
        /// </summary>
        // ----------------------------------------------------------------------
        private GOCompPool<ParticleSystem> GetOrCreatePool
        (
            int variantIndex,
            UnityParticleSystemCueVariant variant
        )
        {
            ...
            if (variant?.Prefab == null)
            {
                ...
                (
                    $"Unity Particle System Cue Variant {variantIndex}에 ParticleSystem Prefab이 필요합니다."
                );
            }
        ...
        private string GetCueName() =>
            asset != null ? asset.name : nameof(UnityParticleSystemCue);
    ...
    #region 정리
        ...
        /// </summary>
        // ----------------------------------------------------------------------
        private void ReleaseLease(Lease<ParticleSystem> poolLease)
        {
            try
            {
                poolLease.Dispose();
        ...
        private void ThrowIfDisposed()
        {
            ...
            throw new ObjectDisposedException(nameof(UnityParticleSystemCue));
        }
